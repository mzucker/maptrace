...
######################################################################
# Primary MapTrace class

class MapTrace:
    """ MapTrace class to trace images and convert to svg"""
    ...
    def filter_type(self, fstr):
        ...
        if operation not in list(fnmap.keys()):
            raise argparse.ArgumentTypeError('invalid operation ' + operation)

        if element == 'box':
            element = self.BOX_ELEMENT
        elif element == 'cross':
            element = self.CROSS_ELEMENT
        else:
    ...
    #
    #     parser.add_argument('video', type=argparse.FileType('rb'),
    #                         metavar='IMAGE.png', nargs='?',
    #                         help='image to approximate')
        ...
        parser.add_argument('-f', '--filter', type=self.filter_type, default=None,
        ...
        if os.path.exists(opts.output_file) and not opts.overwrite:
            if not self.confirm(opts.output_file + ' exists. Overwrite?'):
    ...
    def quantize(self, image, bits_per_channel=None):
    ...
    def get_dominant_color(self, colors, bits_per_channel=None):
        """  Get the dominant color in a list of colors (with optional quantization)"""
        assert colors.shape[-1] == 3

        quantized = self.quantize(colors, bits_per_channel).astype(int)
    ...
    def save_debug_image(self, opts, name, image):
    ...
    def get_mask(self, input_image, opts):
        ...
        self.save_debug_image(opts, 'mask', mask)

        if opts.filter is not None:
            print('applying filter:', opts.filter[0])
            mask = opts.filter[1](mask)
            self.save_debug_image(opts, 'mask_filtered', mask)

        return mask


    def printp(self, *args):
        print(*args, end='')
        sys.stdout.flush()


    def get_labels_and_colors(self, mask, opts):

        if opts.connectivity == '8':
            structure = self.BOX_ELEMENT
        else:
            structure = self.CROSS_ELEMENT
        ...
        replace[idx] = list(range(len(idx)))
        ...
        if opts.min_area > areas[-1]:
            ...
            self.save_debug_image(opts, 'kill_labels', kill_mask)
        ...
        if opts.color_image is not None:
            ...
            for label, (yslc, xslc) in zip(list(range(1, num_labels+1)), slices):
                ...
                if not np.any(lmask):
                    ...
                    colors[label] = self.get_dominant_color(crect[lmask],
        ...
        self.save_debug_image(opts, 'regions', colors[labels])

        self.printp('running DT... ')
        ...
        if opts.debug_images:
            ...
            self.save_debug_image(opts, 'dist', dist)
        else:
            idx = result
        ...
        self.save_debug_image(opts, 'regions_expanded', colors[labels_big[1:-1, 1:-1]])

        return num_labels, labels_big, slices_big, colors


    def follow_contour(self, l_subrect, cur_label,
                       startpoints, pos):

        start = pos
        cur_dir = self.DIR_RIGHT

        contour_info = []

        while True:

            ooffs = self.OPP_OFFSET[cur_dir]
            noffs = self.NEIGHBOR_OFFSET[cur_dir]
            doffs = self.DIAG_OFFSET[cur_dir]
            ...
            if pos == start and cur_dir == self.DIR_RIGHT:
                break
        ...
        cpoints += 0.5 * (self.OPP_OFFSET[cdirs] - self.NEIGHBOR_OFFSET[cdirs] + 1)
        ...
        if len(opp_label_set) == 1:
            ...
            xyi = list(zip(cpoints[:,0], cpoints[:,1], idx))
            imin = min(xyi)
            i = imin[-1]
    ...
    def split_contour(self, cpoints, clabels):
    ...
    def store_contour_edges(self, opts, labels,
        ...
        for opp_label, edge in contour_edges:
            ...
            if key in edge_lookup:
                ...
                if not np.all(edge_list[idx] == edge_to_add):
                    ...
                    self.save_debug_image(opts, 'debug_edge', debug)
    ...
    def _simplify_r(self, opts, p0, edge, output_list):
        ...
        if dmax < opts.edge_tol:
            ...
            self._simplify_r(opts, p0, edge[:idx+1], output_list)
            self._simplify_r(opts, edge[idx], edge[idx+1:], output_list)
        else:
            output_list.append(edge)


    def simplify(self, opts, edge):
        ...
        self._simplify_r(opts, p0, edge[1:], output_list)

        return np.vstack( tuple(output_list) )


    def build_brep(self, opts, num_labels, labels, slices, colors):

        brep = BoundaryRepresentation()

        label_range = list(range(1, num_labels+1))
        ...
        for cur_label, (yslc, xslc) in zip(label_range, slices):
            ...
            while np.any(startpoints):
                ...
                cpoints, clabels = self.follow_contour(l_subrect, cur_label,
                                                  startpoints, pos)

                cpoints += p0

                # split contour into (opp_label, points) pairs
                contour_edges = self.split_contour(cpoints, clabels)
        ...
        if opts.edge_tol > 0:
            ...
            brep.edge_list = [ self.simplify(opts, edge) for edge in brep.edge_list ]
            simplified = True
    ...
    def output_svg(self, opts, orig_shape, brep, colors):
        ...
        with open(opts.output_file, 'w') as svg:
            ...
            cpacked = self.pack_rgb(colors.astype(int))
            cset = set(cpacked)
                    ...
                    for i, contour in enumerate(contours):
                        ...
                        for j, (edge_idx, _, step) in enumerate(contour):
                            ...
                            if np.all(edge == iedge):
                                ...
                                if j == 0:
                                    svg.write('M{},{}'.format(*list(map(num_fmt, edge[0]))))
                                for pt in edge[1:]:
                                    svg.write('L{},{}'.format(*list(map(num_fmt, pt))))
...
def main():
    ...
    input_image = Image.open(mt.opts.image)
    if mt.opts.zoom != 1:
        w, h = input_image.size
        wnew = int(round(w*mt.opts.zoom))
        hnew = int(round(h*mt.opts.zoom))
        resample = Image.LANCZOS if mt.opts.zoom > 1 else Image.LANCZOS
        input_image = input_image.resize((wnew, hnew), resample)
        mt.save_debug_image(mt.opts, 'resized', input_image)
    
    mask = mt.get_mask(input_image, mt.opts)

    # labels is a 2D array that ranges from 0 (background) to
    # num_labels (inclusive), and slices are bounding rectangles for
    # each non-zero label.
    num_labels, labels, slices, colors = mt.get_labels_and_colors(mask, mt.opts)

    brep = mt.build_brep(mt.opts, num_labels, labels, slices, colors)

    mt.output_svg(mt.opts, mask.shape, brep, colors)
